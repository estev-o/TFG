#!/usr/bin/env python3
"""Entrenamiento CNN ordinal (CORAL) con IVR agrupado en 4 clases ordinales."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train CNN ordinal classifier for HPI + IVR agrupado"
    )
    parser.add_argument("--train-csv", default="cnn/splits/train.csv")
    parser.add_argument("--val-csv", default="cnn/splits/val.csv")
    parser.add_argument("--out-dir", default="cnn/runs/baseline_grouped_ivr")
    parser.add_argument(
        "--model",
    ...
    parser.add_argument(
        "--ivr-coarse-bins",
        default="0-1,2-3",
        help="Bins coarse para IVR agrupado en formato '0-1,2-3'. Deben cubrir todo el rango.",
    )
    parser.add_argument(
...
IVR_GROUP_LABEL_MODE = "grouped_4_01_23_45_67"
IVR_GROUP_BINS_RAW: tuple[tuple[int, int], ...] = ((0, 1), (2, 3), (4, 5), (6, 7))
IVR_GROUP_CLASS_LABELS: tuple[str, ...] = ("01", "23", "45", "67")


def group_raw_ivr_label(raw_ivr: int) -> int:
    for group_idx, (start, end) in enumerate(IVR_GROUP_BINS_RAW):
        if start <= raw_ivr <= end:
            return group_idx
    raise ValueError(
        f"IVR fuera del rango esperado 0..7 para agrupacion fija 01/23/45/67: {raw_ivr}"
    )
...
def main() -> None:
    ...
    config_data.update(
        {
            ...
            "num_classes_hpi": num_classes_hpi,
            "num_classes_ivr": num_classes_ivr,
            "ivr_label_mode": IVR_GROUP_LABEL_MODE,
            "ivr_grouping_bins_raw": list(IVR_GROUP_BINS_RAW),
            "ivr_grouping_class_labels": list(IVR_GROUP_CLASS_LABELS),
            "ivr_num_classes_raw": 8,
            "ivr_num_classes_grouped": len(IVR_GROUP_CLASS_LABELS),
    ...
    print(
        "IVR agrupado activo: "
        f"{list(IVR_GROUP_CLASS_LABELS)} desde bins raw={list(IVR_GROUP_BINS_RAW)}"
    )
    if args.target == "both":
        print(
