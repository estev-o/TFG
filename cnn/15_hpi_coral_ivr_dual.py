#!/usr/bin/env python3
"""Entrenamiento CNN con HPI ordinal y IVR dual: aplicabilidad + score."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train CNN with HPI CORAL-like ordinal head + IVR dual head"
    )
    parser.add_argument("--train-csv", default="cnn/splits/train.csv")
    parser.add_argument("--val-csv", default="cnn/splits/val.csv")
    parser.add_argument("--out-dir", default="cnn/runs/baseline_hpi_coral_ivr_dual")
    parser.add_argument(
        "--model",
    ...
    parser.add_argument(
        ...
        choices=["both", "hpi"],
        default="both",
        help="En esta variante no se soporta target=ivr, porque IVR=0 depende de HPI.",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="Peso interno de la loss binaria de aplicabilidad de IVR.",
    )
    parser.add_argument(
        "--ivr-score-loss-weight",
        type=float,
        default=0.5,
        help="Peso interno de la loss de score 1..7 de IVR.",
    )
    parser.add_argument(
        "--ivr-app-threshold",
        type=float,
        default=0.5,
        help="Umbral de aplicabilidad en inferencia para IVR.",
    )
    parser.add_argument(
        "--condition-ivr-on-hpi",
        action="store_true",
        help="Condiciona las cabezas IVR con la salida ordinal de HPI dentro de la red.",
    )
    parser.add_argument(
        "--ivr-conditioning-source",
        choices=["hpi_probs", "hpi_logits"],
        default="hpi_probs",
        help="Que representacion de HPI alimentar a las cabezas IVR condicionadas.",
    )
    parser.add_argument(
        "--ivr-conditioned-hidden-dim",
        type=int,
        default=128,
        help="Dimensionalidad del MLP intermedio para IVR condicionado.",
    )
    parser.add_argument(
        "--ivr-conditioned-dropout",
        type=float,
        default=0.1,
...
def ivr_score_targets_serializable() -> list[Optional[float]]:
    targets: list[Optional[float]] = [None] * 8
    for cls, score in IVR_SCORE_TARGETS_BY_CLASS.items():
        targets[cls] = score
    return targets


def ivr_score_targets_from_labels(labels: torch.Tensor) -> torch.Tensor:
    scores = torch.zeros_like(labels, dtype=torch.float32)
    for cls, score in IVR_SCORE_TARGETS_BY_CLASS.items():
        scores = torch.where(
            labels == cls,
            torch.full_like(scores, fill_value=score),
            scores,
        )
    return scores


def decode_ivr_score_to_class(scores: torch.Tensor) -> torch.Tensor:
    ...
    anchor_scores = torch.tensor(
        [IVR_SCORE_TARGETS_BY_CLASS[int(c)] for c in anchor_classes.tolist()],
        device=scores.device,
        dtype=scores.dtype,
    )
    pred = torch.zeros(scores.shape[0], device=scores.device, dtype=torch.int64)
    distances = (scores.unsqueeze(1) - anchor_scores.unsqueeze(0)).abs()
    idx = distances.argmin(dim=1)
    pred = anchor_classes[idx]
    return pred


def ivr_applicable_targets_from_hpi(labels_hpi: torch.Tensor) -> torch.Tensor:
    return ((labels_hpi > 0) & (labels_hpi < 5)).to(torch.float32)


class KelpScoreDataset(Dataset):
    def __init__(
        self,
        csv_path: str,
...
class HpiCoralIvrDualLoss(nn.Module):
    def __init__(
        self,
        target: str,
        num_classes_hpi: int,
...
def infer_num_classes(
    train_ds: KelpScoreDataset, val_ds: KelpScoreDataset
) -> tuple[int, int]:
    ...
    if num_classes_ivr < 8:
        raise ValueError(
            "Esta variante espera IVR con etiquetas raw 0..7 para mapear el score."
        )
...
def main() -> None:
    ...
    train_ds = KelpScoreDataset(
        args.train_csv,
        train_tfms,
        target=args.target,
        max_samples=args.max_train_samples,
    )
    val_ds = KelpScoreDataset(
    ...
    if args.target == "both":
        ...
        raw_ivr_app = float(args.ivr_app_loss_weight)
        raw_ivr_score = float(args.ivr_score_loss_weight)
        if raw_ivr_app < 0 or raw_ivr_score < 0:
            raise ValueError("Los pesos internos de IVR deben ser >= 0.")
        denom_ivr = raw_ivr_app + raw_ivr_score
        if denom_ivr <= 0:
            raise ValueError("La suma de pesos internos de IVR debe ser > 0.")
        ivr_app_loss_weight = raw_ivr_app / denom_ivr
        ivr_score_loss_weight = raw_ivr_score / denom_ivr
    else:
        both_weight_hpi = 1.0
        both_weight_ivr = 0.0
        ivr_app_loss_weight = 0.0
        ivr_score_loss_weight = 0.0
    ...
    config_data.update(
        {
            "head_type": head_type,
            "hpi_head_impl": "legacy_ordinal_bce_linear",
            "hpi_head_matches_coral_paper_exactly": False,
            "hpi_head_note": (
                "Mantiene exactamente la formulacion usada en 09_train_ordinal.py "
                "(ordinal BCE con K-1 logits). No implementa la capa CORAL estricta "
                "de pesos compartidos del paper."
            ),
            "num_classes_hpi": num_classes_hpi,
    ...
    print(
        ...
        f"classes_hpi={num_classes_hpi} classes_ivr_raw={num_classes_ivr}"
    )
    print(
        "HPI usa exactamente la misma cabeza/loss que 09_train_ordinal.py "
        "(CORAL-like, no CORAL estricto del paper)."
    )
    if args.target == "both":
        print(
